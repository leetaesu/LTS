...
function columnChart(id) {
  ...
  return Highcharts.chart(id, {
    ...
    xAxis: {
      ...
      tickPositions: [-20, -10, 0, 10, 20] // tickPixelInterval: 50,
...
GaugeChart.prototype.init = function (value, leftId, rightId, parentElementName) {
  ...
  labelElement.prepend(value); // 아래 소스가 양쪽 끝 아래, 라운드 시키는 부분 
...
function sellBuyLineChart(period, id, data) {
  ...
  var bgColor = isDark ? '#202024' : '#F7F9FA'; // 차트 데이터에 필요한 내용
...
function dualLineChart() {
  ...
  for (var i = 0; i < sampleDate.length; i++) {
    buyResult.push([sampleDate[i].getTime(), buyList[i]]);
    sellResult.push([sampleDate[i].getTime(), sellList[i]]);
  }

  var buyMin = Math.min.apply(null, buyResult.map(function (el) {
    return el[1];
  }));
  var buyMax = Math.max.apply(null, buyResult.map(function (el) {
    return el[1];
  }));
  ...
  Highcharts.chart('dualLineChart', {
    ...
    yAxis: [{
      ...
      tickPixelInterval: 40,
      plotLines: [plotLineLabel(sell, 'blue'), plotLineLabel(buy, 'red') // {
    ...
    series: [{
      ...
      fillColor: {
        ...
        stops: [[0, '#ffeef4'], [1, 'rgba(255,255,255,0)']]
      },
      yAxis: 1,
      enableMouseTracking: false,
      data: sellResult // crisp : true,
