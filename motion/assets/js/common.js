...
const control = (function(){
  function sortOpt(el){
    const toggleActiveElement = el.find('i');
...
$(".accordion_list > button").on('click', function () {
  ...
  var isExpand = $(this).attr(txtAttrExpend) === 'true';
  // var offsetTop = $(this).offset().top;

  $(this).attr(txtAttrExpend, !isExpand);
  // setTimeout(() => {
...
GaugeChart.prototype.init = function (rateValue, leftId, rightId, parentElementName, originalvalue) {
    ...
    labelElement.text('');
    labelElement.prepend(originalvalue ? originalvalue.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") : rateValue+'<span>%</span>');
...
function sellBuyLineChart(period, id, data) {
  ...
  var highChartOption = {
    ...
    tooltip: {
      ...
      formatter() {
        ...
        var yValue = this.y;
        var windowWidth = window.innerWidth
        var chartWidth = document.querySelector(`#${id}`).clientWidth
        ...
        series.xAxis.addPlotLine({
          ...
          label: {
            ...
            align: 'left',
            useHTML: true,
            text: '<div class="xaxis_label">' + Highcharts.dateFormat(`%y.${dateFormat}`, xValue) + '</div>',
            style: {
                display: 'block',
                height: 31,
                transform: 'rotate(0)'
        ...
        $(`.x-plot-date-line.${id}`).attr('d', $(`.x-plot-date-line.${id}`).attr('d').replace(' 0 ', ' 44 '));
        
        // position을 제대로 못구하는 버그 있음
        setTimeout(function () {
          var priceBoxElement = document.querySelector(`.${id}.close_price_box`);
          
          var parentElement = priceBoxElement.parentNode;
          var grandParentElement = parentElement.parentNode;

          $(`.x-plot-date-line.${id}`).show(0);

          console.log('leftMargin : ', leftMargin)
          priceBoxElement.style.visibility = 'visible';  // 까닥이는 모습을 없애기 위해 사용
          parentElement.style.left = 0;
          grandParentElement.style.left = `${leftMargin}px`;
        }, 0)
        return `
          <div class="${id} close_price_box" style="width: ${chartWidth - rightYAxisWidth}px">
            <span>종가</span>
            <span>${yValue.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}</span>
          </div>
        `
      }
    },
  ...
  return Highcharts.chart(id, highChartOption, function (e) {
    ...
    var bigIndex = 0;
    var smallIndex = 0;
    $(`#${id} .yaxis_label`).each(function(item) {
      var averageObj = {};
    ...
    var bigValue = Math.max.apply(Math, averages.map((item) => item.price));
    bigIndex = averages.findIndex((item) => {
      return item.price === bigValue
    });
    var smallValue = Math.min.apply(Math, averages.map((item) => item.price));
    smallIndex = averages.findIndex((item) => {
    ...
    function positionChange(type) {
      ...
      var selector = $(`#${id} .yaxis_label`).eq(selectIndex)
    ...
    if(averages[smallIndex] != null || averages[smallIndex]!= undefined || averages[smallIndex] == 0 ||
      averages[bigIndex] != null || averages[bigIndex]!= undefined || averages[bigIndex] == 0){
      if (yAxisMin - averages[smallIndex].height < averages[smallIndex].top) {
        if (averages[smallIndex].top - averages[smallIndex].height < averages[bigIndex].top && 
            averages[smallIndex].top + averages[smallIndex].height > averages[bigIndex].top) {
            positionChange('small')
        }
      } else if (yAxisMax + averages[bigIndex].height > averages[bigIndex].top) {
        if (averages[bigIndex].top - averages[bigIndex].height < averages[smallIndex].top && 
            averages[bigIndex].top + averages[bigIndex].height > averages[smallIndex].top) {
            positionChange('big')
        }
      } else {
        if (averages[bigIndex].top - averages[bigIndex].height < averages[smallIndex].top && 
...
function columnVsChart(target, data) {
  ...
  function html(isRise, percent, price) {
      if (isRise) {
          return `
              <b>${parseFloat(percent).toFixed(2)}%</b>
  ...
  selector('.closing_price_box b').text(`${data.closing.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}원`);
